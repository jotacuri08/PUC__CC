...
void insercaoPorCor(Jogador jogadoresDesejados[], int n, int cor, int h){
    for (int i = (h + cor); i < n; i+=h) {
        Jogador tmp = jogadoresDesejados[i];
        int j = i - h;
        while ((j >= 0) && (jogadoresDesejados[j].peso > tmp.peso || 
            (jogadoresDesejados[j].peso == tmp.peso && strcmp(jogadoresDesejados[j].nome, tmp.nome) > 0))) {
            jogadoresDesejados[j + h] = jogadoresDesejados[j];
            j-=h;
        }
        jogadoresDesejados[j + h] = tmp;
    }
}

//=============================================================================
void shellsort(Jogador jogadoresDesejados[], int numJogadoresDesejados) {
    int h = 1;

    do { h = (h * 3) + 1; } while (h < numJogadoresDesejados);

    do {
        h /= 3;
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(jogadoresDesejados, numJogadoresDesejados, cor, h);
        }
    } while (h != 1);
}
...
int main() {
    ...
    shellsort(jogadoresDesejados, numJogadoresDesejados);
