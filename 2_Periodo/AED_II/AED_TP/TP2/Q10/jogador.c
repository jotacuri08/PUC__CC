...
void quicksortRec(Jogador* jogadoresDesejados, int esq, int dir) {
    int i = esq, j = dir;
    Jogador pivo = jogadoresDesejados[(dir + esq) / 2];

    while (i <= j) {
        // Critério primário: estado de nascimento
        while (i <= dir && 
              (strcmp(jogadoresDesejados[i].estadoNascimento, pivo.estadoNascimento) < 0 || 
              (strcmp(jogadoresDesejados[i].estadoNascimento, pivo.estadoNascimento) == 0 && 
               strcmp(jogadoresDesejados[i].nome, pivo.nome) < 0))) {
            i++;
        }

        // Critério primário: estado de nascimento
        while (j >= esq && 
              (strcmp(jogadoresDesejados[j].estadoNascimento, pivo.estadoNascimento) > 0 || 
              (strcmp(jogadoresDesejados[j].estadoNascimento, pivo.estadoNascimento) == 0 && 
               strcmp(jogadoresDesejados[j].nome, pivo.nome) > 0))) {
            j--;
        }
    ...
    if (esq < j)  quicksortRec(jogadoresDesejados, esq, j);
    if (i < dir)  quicksortRec(jogadoresDesejados, i, dir);
}

void quicksort(Jogador* jogadoresDesejados, int n) {
    quicksortRec(jogadoresDesejados, 0, n-1);
}
...
int main() {
    ...
    quicksort(jogadoresDesejados, numJogadoresDesejados);
